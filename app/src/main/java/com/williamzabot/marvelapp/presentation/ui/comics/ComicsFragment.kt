...
class ComicsFragment : Fragment() {
    ...
    private val viewModel2 by activityViewModels<ComicsActivityViewModel>()
    private val comicAdapter by lazy {
        ComicsAdapter { comic ->
            findNavController().navigate(ComicsFragmentDirections.comicToDetail(comic))
            viewModel2.ultimoComicClicado = comic
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (viewModel2.ultimoComicClicado != null) {
            Snackbar.make(
                binding.constraintLayout, viewModel2.ultimoComicClicado?.title.toString(),
